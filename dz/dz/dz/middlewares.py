...
class LoginMidddleware(object):
    ...
    def process_request(self,request,spider):
        if request.url.startswith(spider.login_url):
            spider.logger.info('开始模拟登陆')
            self.driver.get(request.url)  #打开登录页面
            #查找iframe，并切换到frame
            #查找iframe中切换账号的登录图标，并点击
            #再点击账号和密码登录
            #点击登录   注意：时间间隔不要太短，不然不会出现验证码
            iframe=self.driver.find_element_by_xpath('//firame')
            self.driver.switch_to.frame(iframe)
            time.sleep(1)
            qcrode=self.driver.find_element_by_xpath('//div[@class="icon-qrcode"]')
            qcrode.click()
            time.sleep(1)

            self.driver.find_element_by_xpath('//a[@id="tab-account"]').click()
            self.driver.find_element_by_xpath('//*[@id="account-textbox"]').send_keys('18829298582')
            self.driver.find_element_by_xpath('//*[@id="password-textbox"]').send_keys('woaiwojia,.258')
            self.driver.find_element_by_xpath('//*[@id="login-button-account"]').click()
            time.sleep(2)
            self.driver.switch_to.default_content()
            user_info=self.driver.find_element_by_xpath('//*[@class="userinfo_container"]')
            if user_info:
                print('--登陆成功--')
